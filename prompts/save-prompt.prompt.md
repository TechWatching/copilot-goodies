...
6. Create a very concise action-oriented title in kebab-case format that will be used for the slash command (1-3 words, e.g., "generate-unit-tests", "refactor-for-performance", "explain-api-design", etc)
...
9.  Construct the full file path: `${workspaceFolder}/.github/prompts/${promptFileName}.md` where `${promptFileName}` is the concise action-oriented title from step 6
10. Create a new file at the constructed path with the generated prompt content.
